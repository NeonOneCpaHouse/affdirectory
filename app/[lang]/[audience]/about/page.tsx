...
import { localizedAboutText } from './about-data';
...
export default function AboutPage() {
    ...
    const renderIcon = (id: string, className: string) => {
        switch (id) {
            case 'rankings': return <Network className={className} />;
            case 'articles': return <FileText className={className} />;
            case 'knowledge_base': return <BookOpen className={className} />;
            case 'tools_arbitrage': return <Zap className={className} />;
            case 'tools_webmaster': return <Calculator className={className} />;
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                <div className="flex-1 min-w-0">
                    ...
                    <div className="mb-12">
                        ...
                        <div className="space-y-6">
                            {data.offerings.sections.map((section, idx) => (
                                <div key={idx} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm hover:border-sky-300 dark:hover:border-sky-700/70 transition-colors">
                                        ...
                                        <ul className="grid md:grid-cols-2 gap-x-6 gap-y-3 mt-4">
                                            {section.items[currentLang].map((item: any, i: number) => (
                                                <li key={i} className="text-sm">
                                                    <span className="font-semibold text-gray-900 dark:text-white mr-2">{item.label} —</span>
                    ...
                    <div className="bg-sky-600 dark:bg-sky-900 text-white rounded-3xl p-8 mb-8 shadow-sm">
                        ...
                        <p className="text-sky-100 leading-relaxed max-w-3xl mb-6">
                            {data.join.text[currentLang]}
                        </p>
                        {/* We were instructed not to touch email collection for now, so we just show the texts */}
                    </div>
