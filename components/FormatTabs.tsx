...
export default function FormatTabs({ onTabChange, defaultFormat = "webPush" }: FormatTabsProps) {
  ...
  return (
    <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
      <div className="flex gap-2 p-1 bg-sky-100 dark:bg-gray-800/50 rounded-lg min-w-max">
        {formats.map((format) => (
          <button
            key={format}
            onClick={() => handleTabClick(format)}
            className={`whitespace-nowrap px-3 py-2.5 sm:px-4 rounded-md text-sm font-medium transition-colors ${
              activeFormat === format
