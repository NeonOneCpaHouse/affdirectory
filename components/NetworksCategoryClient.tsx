...
export default function NetworksCategoryClient({ categories }: { categories: CategoryItem[] }) {
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                <main className="flex-1 min-w-0">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-8 max-w-2xl">
                        {language === "ru"
                            ? "Просмотрите рекламные сети по формату"
                            : "Browse ad networks by format"}
                    </p>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {categories.map((cat) => (
                            <Link
                                key={cat.key}
                                href={`/${language}/${audience}/networks/${cat.key}`}
