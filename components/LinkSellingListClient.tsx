...
import { getLinkSellingAvgRating } from "@/mock/linkSelling"
import { useLanguage } from "@/context/LanguageContext"
import { useAudience } from "@/context/AudienceContext"

export default function LinkSellingListClient({ entries }: { entries: LinkSellingEntry[] }) {
                ...
                <main className="flex-1 min-w-0">
                    ...
                    <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
                        {entries.map((entry, index) => {
                            ...
                            const avgRating = getLinkSellingAvgRating(entry)
                            return (
                                <>
                                    <Link key={entry.slug} href={`/${language}/${audience}/monetization/link-selling/${entry.slug}`} className="block bg-white dark:bg-gray-800/50 border border-accent-200 dark:border-gray-700/50 rounded-xl p-5 hover:border-accent-500 dark:hover:border-accent-500/50 transition-all group shadow-sm">
