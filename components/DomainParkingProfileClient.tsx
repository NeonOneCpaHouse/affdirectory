...
const ratingTooltips: Record<string, Record<string, string>> = {
    ...
    technology: {
        en: "Measures the quality and reliability of the technology used",
        ru: "Оценивает качество и надёжность используемых технологий",
    },
    security: {
        en: "Measures the level of security and data protection provided",
        ru: "Оценивает уровень безопасности и защиты данных",
    },
    effectiveness: {
        en: "Measures overall effectiveness and revenue potential",
        ru: "Оценивает общую эффективность и потенциал дохода",
    },
}
...
export default function DomainParkingProfileClient({
    entry,
    alternatives,
}: {
    entry: DomainParkingEntry
    alternatives: DomainParkingEntry[]
}) {
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white dark:bg-gray-800/50 border border-accent-200 dark:border-gray-700/50 rounded-xl p-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-6">
                            ...
                            <div>
                                ...
                                <p className="text-gray-600 dark:text-gray-300 text-sm">
                                    {entry.paymentMethods?.slice(0, 3).join(", ") || "—"}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-accent-100 dark:border-gray-700/50 flex flex-wrap items-center gap-4">
                            ...
                            {entry.socials && Object.entries(entry.socials).map(([key, url]) => {
                                ...
                                return (
                                    <a key={key} href={url} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-accent-600 transition-colors" title={social.label}>
                    ...
                    <div className="lg:hidden bg-accent-600 rounded-xl p-6 text-center shadow-md">
                        ...
                        <a href={entry.signupUrl} target="_blank" rel="noopener noreferrer" className="block w-full bg-white text-accent-600 font-medium py-3 rounded-lg hover:bg-accent-50 transition-colors shadow-sm">
                                ...
                                {entry.support.map((contact, idx) => {
                                    ...
                                    return (
                                        <a key={idx} href={contact.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-accent-50 dark:bg-gray-900/50 rounded-lg hover:bg-accent-100 dark:hover:bg-gray-800 transition-colors">
                                            {contact.avatar ? (
                                                <img src={contact.avatar} alt="Support" className="w-10 h-10 rounded-full object-cover" />
                                            ) : (
                                            ...
                                            <div>
                                                <span className="text-gray-900 dark:text-white"><PlatformIcon className="w-5 h-5 inline-block mr-2 text-accent-600" /></span>
                                                <span className="text-sm text-gray-600 dark:text-gray-300 capitalize">{contact.platform}</span>
                                            </div>
                                        </a>
                                    )
                                })}
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                            ...
                            <ul className="space-y-2">
                                {currentPros.map((p, i) => (
                                    <li key={i} className="text-gray-700 dark:text-gray-300 text-sm flex items-start gap-2">
                                        <span className="text-green-600 dark:text-green-500">✓</span>{p}
                                    </li>
                                ))}
                            </ul>
                        </div>
                        <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-xl p-6">
                            ...
                            <ul className="space-y-2">
                                {currentCons.map((c, i) => (
                                    <li key={i} className="text-gray-700 dark:text-gray-300 text-sm flex items-start gap-2">
                                        <span className="text-red-600 dark:text-red-500">✗</span>{c}
                                    </li>
                                ))}
                            </ul>
                        </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    {alternatives.map((alt) => {
                                        ...
                                        return (
                                            <a key={alt.slug} href={`/${language}/${audience}/monetization/domain-parking/${alt.slug}`} className="block bg-white dark:bg-gray-800/50 border border-accent-200 dark:border-gray-700/50 rounded-lg p-3 hover:border-accent-500 transition-colors">
                                                <div className="flex items-center gap-3">
                                                    {alt.logo && <img src={alt.logo} alt={altName} className="w-8 h-8 object-contain" />}
