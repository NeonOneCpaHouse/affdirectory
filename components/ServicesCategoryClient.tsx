...
export default function ServicesCategoryClient({ categories }: { categories: CategoryItem[] }) {
    const { language, t } = useLanguage()
    const { audience } = useAudience()

    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <Breadcrumbs items={[{ label: language === "ru" ? "Сервисы" : "Services" }]} />
            <div className="mb-8">
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                <main className="flex-1 min-w-0">
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
                        {language === "ru" ? "Сервисы" : "Services"}
                    </h1>
                    <p className="text-gray-500 dark:text-gray-400 mb-8 max-w-2xl">
                        {language === "ru"
                            ? "Просмотрите сервисы по категориям"
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {categories.map((cat) => (
                            <Link
                                key={cat.key}
                                href={`/${language}/${audience}/services/${cat.key}`}
