...
import { getServiceAvgRating } from "@/mock/services"
...
export default function ServicesCategoryListClient({ services, categoryKey, categoryLabel }: Props) {
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                <main className="flex-1 min-w-0">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-6">
                        {services.length > 0
                            ? `${services.length} ${language === "ru" ? "сервисов" : "services"}`
                    ...
                    <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
                        {services.map((service, index) => {
                            ...
                            const avgRating = getServiceAvgRating(service)
                            return (
                                <>
                                    <Link
                                        key={service.slug}
                                        href={`/${language}/${audience}/services/${categoryKey}/${service.slug}`}
