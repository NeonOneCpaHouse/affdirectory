...
import { Network } from '@/mock/networks';
...
export default function NetworkCard({ network }: NetworkCardProps) {
    ...
    // Safety check for formatsSupported
    const formats = network.formatsSupported || [];

    const avgRating = formats.length > 0
        ...
        >
            <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white group-hover:text-sky-600 dark:group-hover:text-blue-400 transition-colors">{network.name}</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400">{network.payoutFrequency} • Min ${network.minPayout}</p>
                    </div>
                </div>
