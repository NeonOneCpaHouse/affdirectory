...
export default function RatingStars({ rating, size = 'md', showValue = true }: RatingStarsProps) {
    ...
    return (
        <div className="flex items-center gap-1">
            <div className="flex">
                ...
                {hasHalf && (
                    <svg className={`${starSize} text-yellow-400`} fill="currentColor" viewBox="0 0 20 20">
                        <defs><linearGradient id="half"><stop offset="50%" stopColor="currentColor" /><stop offset="50%" stopColor="#4B5563" /></linearGradient></defs>
            ...
            {showValue && <span className={`text-gray-400 ${size === 'sm' ? 'text-xs' : 'text-sm'}`}>{rating.toFixed(1)}</span>}
        </div>
    );
}
